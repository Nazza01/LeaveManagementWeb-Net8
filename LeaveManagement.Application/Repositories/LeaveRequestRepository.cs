...
namespace LeaveManagement.Application.Repositories
{
    public class LeaveRequestRepository : GenericRepository<LeaveRequest>, ILeaveRequestRepository
    {
        ...
        {
            var leaveRequest = await GetAsync(leaveRequestId);
            leaveRequest.IsApproved = approved;

            if (approved)
            {
                var allocation = await leaveAllocationRepository.GetEmployeeAllocation(
        ...
        public async Task<AdminLeaveRequestViewVM> GetAdminLeaveRequestList()
        {
            ...
            var model = new AdminLeaveRequestViewVM
            {
                ...
                ApprovedRequests = leaveRequests.Count(q => q.IsApproved == true),
                PendingRequests = leaveRequests.Count(q => q.IsApproved == null),
                RejectedRequests = leaveRequests.Count(q => q.IsApproved == false),
